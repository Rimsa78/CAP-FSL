...
from setup import reset_seed, can_prototypical_loss
import time
...
def test_initialize():
    ...
    feature_extractor.load_state_dict(torch.load('Models/AP-5.pth'))
...
def test_loop(meta_test_loader, device, feature_extractor, optimizer):
    ...
    for test_task_data, test_task_labels in meta_test_loader:
        ...
        # Calculate loss, accuracy, and get predictions
        loss, acc, _, predicted_labels, true_labels = can_prototypical_loss(
            feature_vectors_support, 
            support_set_labels, 
        ...
        for class_label in torch.unique(true_labels):
            TP = ((predicted_labels == class_label) & (true_labels == class_label)).sum().item()
            FP = ((predicted_labels == class_label) & (true_labels != class_label)).sum().item()
            TN = ((predicted_labels != class_label) & (true_labels != class_label)).sum().item()
            FN = ((predicted_labels != class_label) & (true_labels == class_label)).sum().item()
