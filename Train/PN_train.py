...
from setup import pn_prototypical_loss, reset_seed
...
def train_epoch(device, meta_train_loader, feature_extractor, optimizer, n_way, s_support):
    ...
    for task_data, task_labels in meta_train_loader:
        ...
        loss, acc, log_p_y, predicted_labels, query_set_label = pn_prototypical_loss(feature_vectors_support, support_set_labels, feature_vectors_query, query_set_labels)
...
def main_train_loop(normal_folder, amd_folder, glaucoma_folder, n_way=3, s_support=5, q_query=5, tasks_per_batch=120, epochs=25):
    ...
    for epoch in range(epochs):
        ...
        if epoch_loss < best_val_loss:
            ...
            torch.save(feature_extractor.state_dict(), 'Models/PN-5.pth')
